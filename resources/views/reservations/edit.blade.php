        ...
        <div class="card">
            ...
            <div class="card-body">
                <form class="row gy-1" method="POST" action="{{ route('reservations.update', $reservation->id) }}">
                        ...
                        <div>
                            ...
                            <select class="form-select @error('user_id') is-invalid @enderror" id="user_id" name="user_id" required>
                                ...
                                @foreach ($users as $user )
                                    <option value="{{ $user->id }}" {{ $user->id == $reservation->user_id ? 'selected' : '' }}>{{ $user->nombre }} {{ $user->apellido }}</option>
                                @endforeach
                            </select>
                            @error('user_id')
                        ...
                        <div>
                            ...
                            <select class="form-select @error('consultant_id') is-invalid @enderror" id="consultant_id" name="consultant_id" required>
                                ...
                                @foreach ($consultants as $consultant )
                                    <option value="{{ $consultant->id }}" {{ $consultant->id == $reservation->consultant_id ? 'selected' : '' }}>{{ $consultant->nombre }} {{ $consultant->apellido }}</option>
                                @endforeach
                            </select>
                            @error('consultant_id')
                                <span class="invalid-feedback" role="alert">
                    ...
                    <div class="col-xxl-3 col-md-6">
                        <div>
                            <label for="total_amount" class="form-label">{{ __('Total a pagar (MX)') }}</label>
